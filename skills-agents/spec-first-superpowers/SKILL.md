...
| `db-journey` / `7pj` | **pgi-core-frontend:** Direct Book long journey — **7 lines** (Marine SM twin `0189`+`0206`, `0191`–`0196`). Keep **`7pj`**; do **not** use `8pj`. Prefer full skill `spec-kit-openspec-superpowers` + `references/db-journey.md` |

## pgi-core-frontend Direct Book (when in that repo)

- **7 product lines**, not 8: `0206` = `MARINE_CARGO_2`, twin of `0189` under same Marine journey (`marineCodes()` / FE `burglary/marine.js`).
- Canonical skill + policy: `~/.claude/skills/spec-kit-openspec-superpowers/` (`pgi-core-policy.md`, `references/db-journey.md`) and repo `.cursor/rules/02-pl-seven-product-scope.mdc`.
- Soft alias: if user says `8pj`, redirect to `7pj` + document twin `0206`.
...
Merge feature→uat accidental deletes + restore + 7pj twin + nested Comm/RI: [references/desk-eli5-merge-delete-restore.md](references/desk-eli5-merge-delete-restore.md)
